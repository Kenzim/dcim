<?php
/**
 * One-click IPMI console launcher.
 *
 * Opens in a new tab from admin/client UI, mints a short-lived RackFlow IPMI
 * proxy ticket, then redirects the browser to the launch URL.
 *
 * Usage: /modules/servers/rackflow/ipmi_open.php?serviceid=<tblhosting.id>
 */
...
if (empty($launch['launch_url'])) {
    ...
    rackflow_log('ipmi_open redirect failed', array(
        'serviceid' => $serviceId,
        'rackflow_service_id' => (int)$rackflowServiceId,
        'error' => $err,
    ));
    http_response_code(502);
    echo 'Unable to open IPMI console: ' . htmlspecialchars($err, ENT_QUOTES, 'UTF-8');
    exit;
}
