<?php
/**
 * One-click IPMI HTML5 KVM console launcher.
 *
 * Opens in a popup window from admin/client UI, mints a short-lived RackFlow
 * KVM launch ticket, then redirects the browser to Rackflow's ``/kvm`` page,
 * which redeems the ticket and renders the HTML5 viewer. BMC credentials
 * never reach the browser.
 *
 * Usage: /modules/servers/rackflow/kvm_open.php?serviceid=<tblhosting.id>
 */
...
$launch = rackflow_mintKvmTicket($apiConfig['url'], $apiConfig['key'], (int)$rackflowServiceId);
if (empty($launch['launch_url'])) {
    $err = !empty($launch['error']) ? (string)$launch['error'] : 'Unknown error';
    rackflow_log('kvm_open redirect failed', array(
        'serviceid' => $serviceId,
        'rackflow_service_id' => (int)$rackflowServiceId,
        'error' => $err,
    ));
    http_response_code(502);
    echo 'Unable to open KVM console: ' . htmlspecialchars($err, ENT_QUOTES, 'UTF-8');
    exit;
}
