<?php
/**
 * One-click Serial-over-LAN console launcher.
 *
 * Opens in a popup window from admin/client UI, mints a short-lived RackFlow
 * SOL launch ticket, then redirects the browser to Rackflow's ``/sol`` page,
 * which redeems the ticket and renders the xterm viewer. BMC credentials
 * never reach the browser.
 *
 * Usage: /modules/servers/rackflow/sol_open.php?serviceid=<tblhosting.id>
 */
...
$launch = rackflow_mintSolTicket($apiConfig['url'], $apiConfig['key'], (int)$rackflowServiceId);
if (empty($launch['launch_url'])) {
    $err = !empty($launch['error']) ? (string)$launch['error'] : 'Unknown error';
    rackflow_log('sol_open redirect failed', array(
        'serviceid' => $serviceId,
        'rackflow_service_id' => (int)$rackflowServiceId,
        'error' => $err,
    ));
    http_response_code(502);
    echo 'Unable to open serial console: ' . htmlspecialchars($err, ENT_QUOTES, 'UTF-8');
    exit;
}
