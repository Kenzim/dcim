...
_MSG_KIND_DHCP_OR_TFTP = "kind must be dhcp or tftp"
...
def _dhcp_tftp_allowed_actions(kind_norm: str) -> set[str]:
...
async def _dhcp_tftp_regen_config(db, location_id: int) -> dict:
...
async def _dhcp_tftp_runner_action(db, location_id: int, kind_norm: str, action_norm: str) -> dict:
    instance = (
...
async def dhcp_tftp_status(location_id: int, kind: str = "dhcp") -> dict:
    ...
    async def work(db, ctx):
        ...
        if kind_norm == "dhcp":
            ...
            raise ValueError(_MSG_KIND_DHCP_OR_TFTP)
        if code != 200:
            raise HTTPException(status_code=status.HTTP_502_BAD_GATEWAY, detail=body)
...
async def dhcp_tftp_control(
    ...
    async def work(db, ctx):
        ...
        if kind_norm not in {"dhcp", "tftp"}:
            raise ValueError(_MSG_KIND_DHCP_OR_TFTP)
        if action_norm not in _dhcp_tftp_allowed_actions(kind_norm):
            raise ValueError(f"action must be one of {sorted(_dhcp_tftp_allowed_actions(kind_norm))}")
        if action_norm == "regen":
            return await _dhcp_tftp_regen_config(db, location_id)
        return await _dhcp_tftp_runner_action(db, location_id, kind_norm, action_norm)
