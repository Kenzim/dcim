<script>
  ...
  let familyForm = { name: '', description: '', service_type: 'vm' };
  let productForm = {
    family_id: '',
    name: '',
    description: '',
  ...
  async function loadData() {
    ...
    error = '';
    try {
      const [familyRows, productRows, vmTemplateRows, permissionSetRows, backupStorageRows, subnetRows] = await Promise.all([
        listProductFamilies(),
        listCatalogProducts(),
        listVmTemplates(),
        listPermissionSets(),
  ...
  async function submitFamily() {
    try {
      await createProductFamily({
        ...
        service_type: familyForm.service_type,
      });
      familyForm = { name: '', description: '', service_type: 'vm' };
  ...
  $: newProductFamily = productForm.family_id ? getFamilyById(Number(productForm.family_id)) : null;
  $: newProductIsProxy = newProductFamily?.service_type === 'http_proxy';
  ...
  function openFamilyEditor(family) {
    const isProxy = family.service_type === 'http_proxy';
    editor = { kind: 'family', id: family.id, title: family.name, code: family.code, serviceType: family.service_type };
  ...
  function openProductEditor(product) {
    ...
    editor = {
      ...
      code: product.code,
      hasFamily: !!family,
      serviceType: family?.service_type || 'vm',
    };
    identityForm = {
  ...
  // Keep the editor's notion of service type in sync if the admin reassigns a
  // product to a different group (vm <-> http_proxy) mid-edit.
  $: if (editor?.kind === 'product') {
    const fam = identityForm.family_id ? getFamilyById(Number(identityForm.family_id)) : null;
    const nextType = fam?.service_type || 'vm';
    if (nextType !== editor.serviceType) {
      editor = { ...editor, serviceType: nextType, hasFamily: !!fam };
      specsForm = nextType === 'http_proxy' ? proxyDefaultsForm({}) : buildSpecsForm({}, !!fam);
  ...
  async function saveEditor() {
    ...
    editorSuccess = '';
    try {
      const isProxy = editor.serviceType === 'http_proxy';
...
<div class="catalog-page">
  ...
  {#if editor}
    <div class="editor-page">
      ...
      {#if editor.serviceType === 'http_proxy'}
        <section class="panel">
          <div class="panel-head">
            <h3>Proxy defaults</h3>
            <p>
              {#if editor.kind === 'product'}
                Overrides applied when this product is used; leave blank to inherit the group default.
              {:else}
                Default IP count / subnet / allocation strategy used when auto-assigning IPs for services in this group.
              {/if}
            </p>
          </div>
          <div class="field-grid spec-grid">
            <FormGroup label="IP count" help="How many IPs to auto-assign from IPAM on service creation (1–32).">
              <input type="number" min="1" max="32" bind:value={specsForm.ip_count} placeholder="default: 1" />
            </FormGroup>
            <FormGroup label="Subnet" help="Optional — restrict auto-assignment to one IPAM subnet.">
            ...
            <tbody>
              {#each visibleFamilies as family (family.id)}
                <tr>
                  ...
                  <td>{family.service_type === 'http_proxy' ? 'HTTP/SOCKS proxy' : 'VM'}</td>
            ...
            <tbody>
              {#each visibleProducts as product (product.id)}
                ...
                <tr>
                  ...
                  <td>{product.family_service_type === 'http_proxy' ? 'HTTP/SOCKS proxy' : 'VM'}</td>
...
{#if showFamilyForm}
  <Modal title="Create Product Family / Group" onClose={() => (showFamilyForm = false)}>
    <FormGroup label="Service type" help="What kind of service products in this group create.">
      <select bind:value={familyForm.service_type}>
        <option value="vm">Virtual machine (Proxmox)</option>
        <option value="http_proxy">HTTP/SOCKS proxy (IPAM)</option>
      </select>
    </FormGroup>
...
{#if showProductForm}
  <Modal title="Create Product" onClose={() => (showProductForm = false)}>
    <FormGroup label="Group">
      <select bind:value={productForm.family_id}>
        <option value="">No group (ungrouped)</option>
        {#each families as fam}
          <option value={fam.id}>{fam.name} ({fam.code}) — {fam.service_type === 'http_proxy' ? 'proxy' : 'vm'}</option>
        {/each}
      </select>
    </FormGroup>
    ...
    {#if newProductIsProxy}
      <FormGroup label="IP count override" help="Leave blank to inherit the group default.">
        <input type="number" min="1" max="32" bind:value={productForm.ip_count} placeholder="default: 1" />
      </FormGroup>
      <FormGroup label="Subnet override" help="Leave blank to inherit the group default.">
        <select bind:value={productForm.subnet_id}>
          <option value="">Inherit from group / any enabled subnet</option>
          {#each ipamSubnets as s}
            <option value={String(s.id)}>{s.name} ({s.cidr})</option>
          {/each}
        </select>
      </FormGroup>
      <FormGroup label="Allocation strategy override">
